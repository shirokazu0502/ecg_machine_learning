...
if __name__ == '__main__':
    ...
    temp_line, = ax.plot(plot_time, plot_buffer[0], color="blue", linewidth=0.5, linestyle="-", label="0")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[1], color="red", linewidth=0.5, linestyle="-", label="1")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[2], color="green", linewidth=0.5, linestyle="-", label="2")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[3], color="black", linewidth=0.5, linestyle="-", label="3")
    lines_sound.append(temp_line)
    plt.ylim(-1 * YLIM, YLIM)
    plt.legend(loc='lower right')

    ax = plt.subplot(4, 1, 2)
    temp_line, = ax.plot(plot_time, plot_buffer[4], color="blue", linewidth=0.5, linestyle="-", label="4")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[5], color="red", linewidth=0.5, linestyle="-", label="5")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[6], color="green", linewidth=0.5, linestyle="-", label="6")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[7], color="black", linewidth=0.5, linestyle="-", label="7")
    lines_sound.append(temp_line)
    plt.ylim(-1 * YLIM, YLIM)
    plt.legend(loc='lower right')

    ax = plt.subplot(4, 1, 3)
    temp_line, = ax.plot(plot_time, plot_buffer[8], color="blue", linewidth=0.5, linestyle="-", label="8")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[9], color="red", linewidth=0.5, linestyle="-", label="9")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[10], color="green", linewidth=0.5, linestyle="-", label="10")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[11], color="black", linewidth=0.5, linestyle="-", label="11")
    lines_sound.append(temp_line)
    plt.ylim(-1 * YLIM, YLIM)
    plt.legend(loc='lower right')

    ax = plt.subplot(4, 1, 4)
    temp_line, = ax.plot(plot_time, plot_buffer[12], color="blue", linewidth=0.5, linestyle="-", label="12")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[13], color="red", linewidth=0.5, linestyle="-", label="13")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[14], color="green", linewidth=0.5, linestyle="-", label="14")
    lines_sound.append(temp_line)
    temp_line, = ax.plot(plot_time, plot_buffer[15], color="black", linewidth=0.5, linestyle="-", label="15")
    lines_sound.append(temp_line)
    plt.ylim(-1 * YLIM, YLIM)
    plt.legend(loc='lower right')
                ...
                for i in range(SPI_DLEN):
                    for ch in range(NUM_CH):
                        ...
                        if ADC_BIT == 24:
                            ...
                            val[ch] = two_comp((rec_vals[0 + startidx] << 16) + (rec_vals[1 + startidx] << 8) + rec_vals[2 + startidx], ADC_BIT)
                        else:
                            startidx = i * 2 * NUM_CH + ch * 2
                            val[ch] = two_comp((rec_vals[0 + startidx] << 8) + rec_vals[1 + startidx], ADC_BIT)
                    ...
                    if count_data == CHUNK:
                        for ch in range(NUM_CH):
                            lpf_out, lpf_zi_ = signal.lfilter(lpf_b, lpf_a, data_buffer[ch], zi=lpf_zi_list[ch])
                            lpf_zi_list[ch] = lpf_zi_
                            hpf_out, hpf_zi_ = signal.lfilter(hpf_b, hpf_a, lpf_out, zi=hpf_zi_list[ch])
                            hpf_zi_list[ch] = hpf_zi_
                            plot_buffer[ch] = np.hstack([plot_buffer[ch][CHUNK:CHUNK * PLOT_LEN], lpf_out])
