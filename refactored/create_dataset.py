...
from settings import DEFAULT_OUTPUT_DIR, DEFAULT_SAMPLING_RATE
...
def main(args):
    ...
    # リサンプリングして長さを揃える
    df_12ch_resampled = resample_dataframe(
        df_12ch_synced, args.rate_12ch, args.resample_rate
    )
    df_16ch_resampled = resample_dataframe(
        df_16ch_synced, args.rate_16ch, args.resample_rate
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="ECGデータセット作成スクリプト（同期・リサンプリング機能付き）"
    )
    parser.add_argument(
        "--input_12ch", type=str, required=True, help="入力12誘導ECGのCSVパス"
    )
    parser.add_argument(
        "--input_16ch", type=str, required=True, help="入力16誘導センサーのCSVパス"
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--rate_12ch", type=int, default=500, help="12誘導ECGのサンプリングレート"
    )
    parser.add_argument(
        "--rate_16ch",
        type=float,
        default=122.06,
        help="16誘導センサーのサンプリングレート",
    )
    parser.add_argument(
        "-rr", "--resample_rate", type=int, default=500, help="リサンプリング後のレート"
    )
    parser.add_argument(
        "--target_12ch",
        type=str,
        default="A2",
        help="同期に使用する12誘導のチャンネル名",
    )
    parser.add_argument(
        "--target_16ch",
        type=str,
        default="ch_1",
        help="同期に使用する15誘導のチャンネル名",
    )
    parser.add_argument(
        "-s", "--heartbeat_seconds", type=float, default=0.8, help="切り出す心拍の秒数"
    )
    parser.add_argument(
        "--debug_plot", action="store_true", help="デバッグ用のプロットを表示する"
    )
