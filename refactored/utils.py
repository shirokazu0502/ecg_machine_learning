...
def plot_fig_test_name_8ch_2row(
    ...
    for p in range(batch_size_num):
        ...
        for q in range(ecg_ch):
            ...
            ax.plot(xticks, recon_data, color="red", linewidth=1.0, linestyle="-", label="predict")
            ax.plot(xticks, xo_data, color="blue", linewidth=1.0, linestyle="-", label="ECG")
        ...
        save_path = os.path.join(args.fig_root, str(ts), f"test_{label_name[p]}.png")
