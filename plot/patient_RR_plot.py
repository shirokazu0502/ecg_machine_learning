...
class ArrayComparator:
    ...
    def create_corr_and_blanalt_plot(self, cut_time, loop_num):
        ...
        axes[1].legend()
        # グラフ全体のタイトルとレイアウト調整
        fig.suptitle(f"patient3 corr and blanaltman plot", fontsize=16)
        plt.tight_layout(rect=[0, 0, 1, 0.95])

        # 保存と表示
        combined_path = (
            DATA_DIR
            + f"/RRI_plot/patient3/corr_blanalt_cuttime{cut_time:.0f}_{loop_num}.png"
        )
        plt.savefig(combined_path)
        plt.show()
        plt.close()
        con_all_data.to_csv(DATA_DIR + f"/RRI_plot/patient3/{loop_num}.csv")
...
if __name__ == "__main__":
    ...
    args.raw_datas_dir = RAW_DATA_DIR + "/patient_data/patient3/RR_plot"
