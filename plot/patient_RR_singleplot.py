...
def find_nearest(row, df1_time, df1_diff):
    ...
    if nearest_idx > 0:
        tmp_abs_diff = abs(df1_diff.iloc[nearest_idx - 1] - row["diff"])
        if tmp_abs_diff < min_abs_diff and tmp_abs_diff < 0.01:
            min_abs_diff = tmp_abs_diff
            nearest_idx = nearest_idx - 1
    if nearest_idx > 1:
        tmp_abs_diff = abs(df1_diff.iloc[nearest_idx - 2] - row["diff"])
        if tmp_abs_diff < min_abs_diff and tmp_abs_diff < 0.01:
            min_abs_diff = tmp_abs_diff
            nearest_idx = nearest_idx - 2
    if nearest_idx < (len(df1_diff) - 1):
        tmp_abs_diff = abs(df1_diff.iloc[nearest_idx + 1] - row["diff"])
        if tmp_abs_diff < min_abs_diff and tmp_abs_diff < 0.01:
            min_abs_diff = tmp_abs_diff
            nearest_idx = nearest_idx + 1
    if nearest_idx < (len(df1_diff) - 2):
        tmp_abs_diff = abs(df1_diff.iloc[nearest_idx + 2] - row["diff"])
        if tmp_abs_diff < min_abs_diff and tmp_abs_diff < 0.01:
            min_abs_diff = tmp_abs_diff
            nearest_idx = nearest_idx + 2
...
class ArrayComparator:
    ...
    def find_best_cut_time(self):
        ...
        for i in range(large_size - small_size + 1):
            if (
                ...
                continue
            # nearest_idxs = []
            # # 時刻同期にあたって比較する提案センサの時間(20ピーク数ぐらい多めに取っておく)
            # split_time2 = time2[i : len(time1) + 20].reset_index(drop=True)
            # print(split_time2)
            # print(split_time2.iloc[0])
            # print("aaaaaaaaaaaaa")
            # split_time2 = (
            #     split_time2 - split_time2.iloc[0] + time1.iloc[0]
            # )  # time1に合わせる
            # for t1 in time1:
            #     nearest_idx = (split_time2 - t1).abs().idxmin()
            #     nearest_idxs.append(nearest_idx)
            # # nearest_idx = (time1 - time2).abs().idxmin()
            # print(len(nearest_idxs))
            # print(nearest_idxs)
            # time.sleep(10)
    ...
    def peak_diff_plot_move(self, cut_time):
        ...
        print(best_index, diff2[best_index], diff1[0])
        print(
            best_index,
            diff2[best_index + len(diff1) - 10 : best_index + len(diff1)],
            diff1[-10:],
        )
        # print(diff1)
        # print(diff2)
    ...
    def create_corr_and_blanalt_plot(self, cut_time, loop_num):
        ...
        axes[1].legend()
        # グラフ全体のタイトルとレイアウト調整
        fig.suptitle(f"patient6 corr and blanaltman plot", fontsize=16)
        plt.tight_layout(rect=[0, 0, 1, 0.95])

        # 保存と表示
        combined_path = (
            DATA_DIR
            + f"/RRI_plot/patient6/corr_blanalt_cuttime{cut_time:.0f}_{loop_num}.png"
        )
        plt.savefig(combined_path)
        # plt.show()
        plt.close()
        con_all_data.to_csv(
            DATA_DIR + f"/RRI_plot/patient6/{loop_num}_corr{corr:.2f}.csv"
        )
