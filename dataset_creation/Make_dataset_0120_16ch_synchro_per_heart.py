...
def peak_sc_plot(dataframe, RATE, TARGET):
    ...
    plt.title(TARGET, fontsize=24, fontweight="bold")
    plt.xlabel("time(s)", fontsize=20, fontweight="bold")

    plt.savefig("peak_sc_plot_{}.svg".format(TARGET))
    # print(sc)
    plt.show()
    plt.close()
    # print(sc)
...
class ArrayComparator:
    ...
    def peak_diff_plot(self):
        ...
        # データ1のプロット
        plt.plot(time1, diff1, label="A2", color="r")
        plt.scatter(time1, diff1, label="A2", color="r")
        # データ2のプロット
        plt.plot(time2, diff2, label="ch_1", color="b")
        plt.scatter(time2, diff2, label="ch_1", color="b")
        # グラフのタイトルと凡例
        plt.title("compare of peak time diff")
        plt.legend(fontsize=16)
        # 軸ラベルの設定
        plt.xlabel("time(s)", fontsize=20, fontweight="bold")
        plt.ylabel("diff(s)", fontsize=20, fontweight="bold")

        plt.savefig("peak_time_diff_compare.svg")
        # グラフの表示
        plt.show()
        plt.close()
    ...
    def peak_diff_plot_move(self, cut_time):
        cut_time, min_mse, _ = self.find_best_cut_time()
        time1, time2, diff1, diff2 = self.cul_diff()
        ...
        plt.plot(time1_v2, diff1, label="A2_move", color="g")
        plt.scatter(time1_v2, diff1, label="A2_move", color="g")
        # グラフのタイトルと凡例
        plt.title("compare of peak time diff")
        plt.legend(fontsize=16)
        ...
        plt.savefig("peak_time_diff_compare_move.svg")
        # グラフの表示
        plt.show()
        plt.close()
...
class AutoIntegerFileHandler:
    ...
    def write_integer(
        ...
        # #    file.write("TARGET_CHANNEL_16ch="+str(self.ch))
        #    file.write(str(target_16ch)+'\n')
...
def find_qrs_boundary(
    boundary_type, signal, start_idx, end_idx, noise_level, threshold_multiplier=3
):
    """
    信号の傾きを元に、QRS波の開始点または終了点を動的に見つける統合関数。
    'offset'検出時は、start_idxにs_peakを渡すことを想定。
    """
    threshold = noise_level * threshold_multiplier
    if boundary_type == "onset":
        # --- QRS開始点 (Onset) の検出ロジック --- (変更なし)
        if start_idx >= end_idx:
            return start_idx

        slopes = np.abs(np.diff(signal[start_idx : end_idx + 1]))
        significant_slopes = np.where(slopes > threshold)[0]

        if len(significant_slopes) > 0:
            first_steep_point = significant_slopes[-1]
            return start_idx + first_steep_point
        else:
            return start_idx

    elif boundary_type == "offset":
        # --- QRS終了点 (Offset) の検出ロジック --- (s_peak探索を削除し簡略化)
        # 探索範囲: start_idx(s_peak) から end_idx(t_onset) まで
        if start_idx >= end_idx:
            return end_idx
        ...
        slopes = np.abs(np.diff(signal[start_idx : end_idx + 1]))
        flat_slopes = np.where(slopes < threshold)[0]

        if len(flat_slopes) > 0:
            first_flat_point = flat_slopes[0]
            return start_idx + first_flat_point
        else:
            return end_idx

    else:
        raise ValueError(
            "boundary_typeは 'onset' または 'offset' である必要があります。"
        )
    ...
    def cut_heartbeats(self, file_path, ch, cut_min_max_range, args):
        ...
        for i, center_idx in enumerate(center_idxs):
            ...
            if start_idx < p_indexs_onsets[i]:
                ...
                t_offset = t_indexs_offsets[i] - start_idx
                p_offset = p_indexs_offsets[i] - start_idx
                t_onset = t_indexs_onsets[i] - start_idx
                p_peak = p_indexs_peaks[i] - start_idx
                q_peak = q_indexs_peaks[i] - start_idx
                s_peak = s_indexs_peaks[i] - start_idx
                t_peak = t_indexs_peaks[i] - start_idx
            # ファイル読み込みの場合(400のデータ長から、p_onset,t_offsetなどの位置を取得)
            else:
                p_onset = p_indexs_onsets[i]
                t_offset = t_indexs_offsets[i]
                p_offset = p_indexs_offsets[i]
                t_onset = t_indexs_onsets[i]
                p_peak = p_indexs_peaks[i]
            ...
            for j, column in enumerate(data.columns):
                ...
                if column == "A2":
                    ...
                    # 1. X軸となる時間配列を作成 (0から始まり、信号の長さ分だけ作成)
                    # 1. 時間軸の作成
                    ...
                    plt.plot(
                        ...
                        color="blue",
                        alpha=0.5,  # 重なりを見やすくするために少し薄くするのもアリです
                        zorder=1,
                    )

                    # 3. フィッティングに使った点をプロット (重要: ここを秒単位に変換)
                    plt.scatter(
                        np.array(baseline_indices) / RATE,  # ここでRATEで割る！
                        baseline_values,
                    ...
                    plt.title("Baseline Correction Debug Plots")
                    plt.xlabel("Time (s)", fontsize=20, fontweight="bold")
...
# クリックイベントで座標を取得する関数
def handle_click(event, coords, max_points=8):
    """
    クリックイベントで座標を取得する関数。

    Parameters:
        event: マウスクリックイベント。
        coords (list): クリックされた座標を格納するリスト。
        max_points (int): 最大取得ポイント数。
    ...
    if event.xdata is not None:
        ...
        print(f"{labels[len(coords) - 1]} は {coords[-1]} です。")
        if len(coords) >= max_points:  # 最大ポイント数に達したら終了
            plt.close()
...
def plot_and_select_all_points(ecg_all, rpeak, window=200):
    ...
    Parameters:
        ecg_all (pd.DataFrame): ECGデータフレーム。
        rpeak (int): R波のピークインデックス。
        window (int): R波を中心とした表示範囲。
    ...
    for i in range(rpeak_num):
        ...
        if rpeak > int(0.5 * time_length * sampling_rate) and rpeak < len(
            ...
            # 各波検出がうまくいかない被験者の場合
                ...
                #     ]
                # )
                window = int(RATE * time_length / 2)  # 0.4秒
                ...
                if None in points:
                    continue  # ピーク検出がおかしい（T波ピークをR波ピークと間違えてるなど）場合はスキップ
                # data_list に特徴点を格納
                data_list.append(
                    [
                        p_Onset_ele,
...
def create_15ch_variations(args):
    ...
    for base_ch_num in base_ch_nums:
        ...
        target_dir_name = f"15ch_diff_from_{base_ch_name}_{args.pos}"
...
def main(args):
    ...
    if handler.check_file() == False:  # 同期するためのファイルが存在していないとき。
        ...
        # 同時計測をしている場合⇒最適サンプリング周波数を求める　していない⇒122.06Hz固定
        simul_measument_flg = int(input("同時計測はしていますか？ yes=1, no=0: "))
        if simul_measument_flg == 1:
            ...
            for rate_candidate in rate_candidates:
                ...
                if reverse == "off":
                    ...
                    df_16ch_reverse[TARGET_CHANNEL_16ch] = (
                        -1
                    ) * df_resample_16ch.copy()[TARGET_CHANNEL_16ch]
            ...
            best_rate = 122.06
            df_resample_16ch = ecg_clean_df_16ch(df_16ch=df_16ch.copy(), rate=best_rate)
            df_resample_16ch = linear_interpolation_resample_All(
                df=df_resample_16ch.copy(),
                sampling_rate=best_rate,
                new_sampling_rate=RATE,
            )
            if reverse == "off":
                sc_16ch = peak_sc_16ch(
            ...
            min_combined_score = mse + score_weight * final_peak_diff
            best_cut_time = cut_time
