...
def main(args):
    ...
    df_16ch=pd.read_csv(dir_path+"db_patient2_16ch_data_2500.csv", header=None)
    ...
    df_12ch=pd.read_csv(dir_path+"improve_FY_5179163_fukuda12ecg_csv.csv")
    ...
    if(handler.check_file()==False):#同期するためのファイルが存在していないとき。
        ...
        RATE_12ch = 500
        df_12ch_pf = ecg_clean_df_12ch(df_12ch=df_12ch.copy(),rate=RATE_12ch)
        df_resample_12ch=linear_interpolation_resample_All(df=df_12ch_pf.copy(),sampling_rate=RATE_12ch,new_sampling_rate=RATE)
...
if __name__ == "__main__":
    ...
    args.cut_min_max_range=[1.0,10.0]
    args.reverse='off'
    ...
    args.raw_datas_dir=RAW_DATA_DIR+"/extract_patient_data/patient2/{}".format(args.dir_name)
