...
def convert_8_to_12ch(args,df_data):
    ...
    df2["aVR"]=-(df2["A1"]+df2["A2"]/2.0)#これで変換しなおしてデータを作り直す。
    df2["aVL"]=(df2["A1"]-df2["A3"]/2.0)
    df2["aVF"]=(df2["A2"]+df2["A3"]/2.0)
    print(df2)
    new_order=["A1","A2","A3","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    df2=df2[new_order]
    dt=1.0/500.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--time_length", type=int, default=24)#24 or 10を選ぶ。若干バイナリの読み方変わる。BzとMFER format　ツールで見比べる。
    ...
    # parser.add_argument("--date", type=str, default='0621')
    # parser.add_argument("--date", type=str, default='0710')
    ...
    args.test_images_path_12lead=TEST_DIR+'/raw_datas_test'+'/'+DATASET_MADE_DATE+'/12lead_raw/'
    args.raw_datas_dir=RAW_DATA_DIR+"/0_packetloss_data_{}/{}".format(DATASET_MADE_DATE,args.dir_name)
