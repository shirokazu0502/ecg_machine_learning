...
class MultiPlotter_both:
    ...
    def multi_plot_12ch_15ch_with_sc_2(self, xmin, xmax, ylim, sc, ch, png_path):
        ...
        for i in range(15):
            ax2.plot(plot_time, self.df15[self.df15.columns[i]], linewidth=1.5, linestyle="-", label=self.df15.columns[i])
        for j in sc[0]:
...
def plot_heartbeats_sotoume(data,num,p_onset,t_offset):
    ...
    ax1.scatter(p_onset/500,df_12ch_data.loc[p_onset,"A2"],marker='^',alpha=0.7,label="P onset",c='b')
    ax1.scatter(t_offset/500,df_12ch_data.loc[t_offset,"A2"],marker='v',alpha=0.7,label="T offset",c='forestgreen')
    for axis in ['top','bottom','left','right']:
        ax1.spines[axis].set_linewidth(axis_line_width)
    ...
    for i in range(15):
        ...
        ax2.plot(plot_time, df_15ch_data[df_15ch_data.columns[i]], linewidth=1.5, linestyle="-", label=df_15ch_data.columns[i])
...
class HeartbeatCutter_prt:
    ...
    def cut_heartbeats(self,file_path,ch,cut_min_max_range,args):
        ...
        append_to_csv(filename=args.dataset_output_path+"/pt_time_all_{}s.csv".format(str(self.time_length)),data=pt_info)
        dataset_num_to_csv(filename=args.dataset_output_path+"/dataset_num_{}s.csv".format(str(self.time_length)),data=data_num_info)
...
#R波
    ...
    compare_path='{}/{}/T_Offsets'.format(args.test_images_path,args.dataset_made_date)
    create_directory_if_not_exists(compare_path)
    plt.savefig(compare_path+"/12ch"+args.type+'.png')
    if(DEBUG_PLOT==True):
        plt.show()
    ...
    for i in range(rpeak_num):
        ...
        if(rpeak>int(0.5*time_length*sampling_rate) and rpeak<12000-int(0.5*time_length*sampling_rate)):
            ...
            # print(t_Peaks_ele)
            # print(t_Offset_ele)
            ele_list=[p_Onset_ele,p_Peaks_ele,p_Offset_ele,q_Peaks_ele,rpeak,s_Peaks_ele,t_Onset_ele,t_Peaks_ele,t_Offset_ele] #各ピークのインデックス
...
def main(args):
    ...
    df_16ch=pd.read_csv(dir_path+"db_patient_16ch_data_10000.csv", header=None)
    ...
    df_12ch=pd.read_csv(dir_path+"implove_KT_4794335_fukuda12ecg_csv.csv", encoding='utf-8')
    ...
    if(handler.check_file()==False):#同期するためのファイルが存在していないとき。
        ...
        min_mse = float('inf')  # 初期値として最大値を設定
        mse_list=[]
        for rate_12ch in frange(120.0, 270.0, 0.01):
            tmp_df_12ch_pf = ecg_clean_df_12ch(df_12ch=df_12ch.copy(),rate=rate_12ch)
            df_resample_12ch=linear_interpolation_resample_All(df=tmp_df_12ch_pf.copy(),sampling_rate=rate_12ch,new_sampling_rate=RATE)
            tmp_df_12ch_pf=df_resample_12ch.copy()
            tmp_sc_12ch=peak_sc(tmp_df_12ch_pf.copy(),RATE=rate_12ch,TARGET=TARGET_CHANNEL_12CH)
            comparator=ArrayComparator(sc_15ch=sc_15ch,sc_12ch=tmp_sc_12ch,cut_min_max_range=cut_min_max_range)
            tmp_cut_time, mse=comparator.find_best_cut_time()
            mse_list.append(mse)
            if mse<min_mse:
                min_mse = mse
                cut_time = tmp_cut_time
                target_rate = rate_12ch
                sc_12ch = tmp_sc_12ch
                df_12ch_pf = tmp_df_12ch_pf
        print(mse_list)
        print(f"{target_rate}だよーーーーーーーーーーーーーー")
        # print(sc_12ch)
        # input()
        peak_sc_plot(df_12ch_pf.copy(),RATE=target_rate,TARGET=TARGET_CHANNEL_12CH)
    ...
    if(DEBUG_PLOT==True):
        ...
        Plot_12ch.multi_plot_12ch_15ch_with_sc_2(xmin=0,xmax=5,ylim=0,sc=sc_12ch,ch=TARGET_CHANNEL_15CH,png_path=args.png_path+'12chsc')
        plt.show()
        plt.close()
        # input()
    ...
    heartbeat_cutter_prt.cut_heartbeats(file_path=args.dataset_output_path+'/'+args.output_filepath,ch=TARGET_CHANNEL_15CH,cut_min_max_range=cut_min_max_range,args=args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reverse", type=str, default='')#onだと波形逆さまにしてピーク検出。これはシートセンサを逆向きに貼ったとき
    ...
    args.name=''
    args.date=''
    args.peak_method='cwt'#neurokitのピーク検出アルゴリズムについてcwtかpeakがある。
    args.pos='0'
    args.type=''
    ...
    args.cut_min_max_range=[1.0,30.0]
    args.reverse='off'
    args.type='{}_{}_{}'.format(args.name,args.date,args.pos)
    args.dir_name='{}'.format(args.name)
    ...
    args.raw_datas_dir=RAW_DATA_DIR+"/extract_patient_data/patient1/{}".format(args.dir_name)
    args.dataset_output_path=PROCESSED_DATA_DIR+"/patient_data{}".format(args.dataset_made_date)
    args.test_images_path=TEST_DIR+'/raw_patient_datas_test'
    main(args)
    # dataset_images_path=''
    # dataset_count_path=''