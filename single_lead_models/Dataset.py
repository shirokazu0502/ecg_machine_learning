...
def Dataset_setup_8ch_pt_augmentation(
    ...
    PGV_set, ECG_set, label_set, pt_set = [], [], [], []

    directory_path = f"/mnt/ecg_project/data/processed/{Dataset_name}"
    all_subject_dirs = get_directory_names_all(directory_path)

    # Filter directories to include only those from the specified subjects
    dirs_to_load = [
        d for d in all_subject_dirs if any(sub in d for sub in include_subjects)
    ]

    all_output_cols = ["A1", "A2", "V1", "V2", "V3", "V4", "V5", "V6"]
    ...
    if target_lead is None:
        ...
        ecg_ch_num = 1
        if target_lead not in all_output_cols:
            raise ValueError(f"target_lead '{target_lead}' is not a valid ECG lead.")
        output_cols = [target_lead]
    ...
    for subject_dir in dirs_to_load:
        for base_ch in base_channels:
            path_to_dataset = os.path.join(directory_path, subject_dir, base_ch)
            for i in range(dataset_num):
                path = os.path.join(
                    path_to_dataset, ave_path, f"dataset_{str(i).zfill(3)}.csv"
                )
                pt_path = os.path.join(
                    path_to_dataset, ave_path, f"ponset_toffset_{str(i).zfill(3)}.csv"
                )

                if not (os.path.isfile(path) and os.path.isfile(pt_path)):
                    continue

                label_name = f"{subject_dir.replace('/', '_')}_dataset{str(i).zfill(3)}"
                data = pd.read_csv(path, header=0)
                df_pt = pd.read_csv(pt_path)
                pt_array = df_pt.iloc[0].to_numpy()
                ...
                PGV_tensor = torch.FloatTensor(data_mul.T.values).reshape(
                    -1, num_channels, datalength
                )
                ECG_tensor = torch.FloatTensor(data_ecg.T.values).reshape(
                    -1, ecg_ch_num, datalength
                )

                PGV_set.append(normalize_tensor_data(PGV_tensor))
                ECG_set.append(normalize_tensor_data(ECG_tensor))
                label_set.append(label_name)
                pt_set.append(pt_array)

                if not augmentations_to_apply or target_lead is None:
                    continue
                ...
                for aug_key in augmentations_to_apply:
                    ...
                    for rate in extation_rates:
                        try:
                            aug_ECG, aug_PGV, aug_label, aug_pt = aug_func(
                                PGV_tensor[0],
                                ECG_tensor[0],
                                pt_array,
                                label_name,
                            ...
                            PGV_set.append(normalize_tensor_data(aug_PGV))
                            ECG_set.append(normalize_tensor_data(aug_ECG))
                            label_set.append(aug_label)
                            pt_set.append(aug_pt)
    ...
    if not PGV_set:
        subject_str = ", ".join(include_subjects)
        raise FileNotFoundError(
            f"No data found for the specified subjects: {subject_str}"
        )

    PGV_set = torch.cat(PGV_set, dim=0)
    ECG_set = torch.cat(ECG_set, dim=0)
    ...
    # This function now returns a single dataset based on the included subjects
    return dataset
