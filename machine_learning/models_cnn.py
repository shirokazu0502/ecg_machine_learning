...
class CNN(nn.Module):
    def __init__(self, datalength, convlayer_sizes, fclayer_sizes):
        super().__init__()
        self.datalength = datalength
        self.conv_layer_sizes = convlayer_sizes
        #if self.conditional:
        #    layer_sizes[0] += num_labels

        self.MLP_1 = nn.Sequential().to(device)
        self.MLP_2 = nn.Sequential().to(device)
        self.MLP_3 = nn.Sequential().to(device)
        self.MLP_4 = nn.Sequential().to(device)

        if len(convlayer_sizes) != 0:
            for i, (conv_param_in, conv_param_out) in enumerate((zip(convlayer_sizes[:-1], convlayer_sizes[1:]))):# conv_layer_sizesの二個ずつペアで入力、出力で読み取ってる
        ...
        for i, (in_size, out_size) in enumerate(zip(fclayer_sizes[:-1], fclayer_sizes[1:])):
            self.MLP_2.add_module(
                name="L{:d}".format(i), module=nn.Linear(in_size, out_size))
            self.MLP_2.add_module(name="A{:d}".format(i), module=nn.ReLU())

        # self.linear_pt = nn.Linear(fclayer_sizes[-1], 400).to(device)#500Hzで0.8のインデックス
        self.MLP_3.add_module(name="linear",module=nn.Linear(fclayer_sizes[-1], 400))#500Hzで0.8のインデックス
        # self.MLP_3.add_module(name="softmax",module=nn.Softmax())#500Hzで0.8のインデックス

        self.MLP_4.add_module(name="linear",module=nn.Linear(fclayer_sizes[-1], 400))#500Hzで0.8のインデックス
        # self.MLP_4.add_module(name="softmax",module=nn.Softmax())#500Hzで0.8のインデックス
