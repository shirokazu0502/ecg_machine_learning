...
def weighted_average_join(
    ...
    """
    P波、R波、T波の再構成波形（2D）を、オーバーラップ区間で加重平均して結合する関数。
    Args:
        recon_x_p (torch.Tensor): P波成分の再構成波形 (channels, length)
        recon_x_r (torch.Tensor): R波成分の再構成波形 (channels, length)
        recon_x_t (torch.Tensor): T波成分の再構成波形 (channels, length)
        (その他の引数は同じ)
    Returns:
        torch.Tensor: 結合されたECG波形 (channels, length)
    """
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(
            model.state_dict(),
            os.path.join(base_path, "model_pth", "vae_sep.pth"),
        )
...
if __name__ == "__main__":
    current_time = "0825_15ch_arrange_direction"
