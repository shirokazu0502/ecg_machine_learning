...
def test_lstm(model, test_loader, device, args, exp_dir):
    model.eval()
    label_temps = []
    test_val_all_mae = []
...
def main():
    ...
    writer = SummaryWriter(
        log_dir=f"runs/{args.current_time}/{args.TARGET_NAME}/LSTM_log"
    )
    ...
    output_size = args.ecg_ch_num
    datalength = args.datalength

    lstm_model = LSTMModel(
        input_size, hidden_size, num_layers, output_size, datalength
    ).to(device)
    ...
    if args.mode == "train":
        ...
        test_loader = DataLoader(
            test_dataset, args.val_batch_size, shuffle=False
        )  # Using test_loader for validation for now

        optimizer = torch.optim.Adam(lstm_model.parameters(), lr=args.learning_rate)
        scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=20, T_mult=2)
        criterion = (
            utils.loss_fn_lstm
        )  # Using custom LSTM loss with smoothness regularization
        ...
        torch.save(
            ...
            os.path.join("model_pth", "lstm_model.pth"),
        )
        torch.save(
            lstm_model.state_dict(),
            os.path.join(args.fig_root, str(ts), "lstm_model.pth"),
        )
        ...
        try:
            ...
            print("Loaded pre-trained model from model_pth/lstm_model.pth")
        ...
        # Create MAE folder if it doesn't exist
        utils.create_directory_if_not_exists(args.mae_folder)
        ...
        data_to_write = {
            ...
            "pearson_all": pearson_all_mean_score,
        ...
        output_file = os.path.join(
            args.mae_folder
            + "/MAE_leave_1_out_{}_augumentation={}.csv".format(
                args.Dataset_name,
                args.DataAugmentation,
            )
        )
