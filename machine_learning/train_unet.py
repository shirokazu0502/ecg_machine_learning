...
def train_unet(
    ...
    for epoch in range(epochs):
        ...
        total_train_corr_loss = 0.0
        model.train()
        for x, xo, _, _ in train_loader:
            x, xo = x.to(device), xo.to(device)
            optimizer.zero_grad()
            outputs = model(x)
            comb_loss, mse_loss, corr_loss = criterion(outputs, xo)
            comb_loss.backward()
            optimizer.step()
            total_train_loss += comb_loss.item()
            total_train_mse_loss += mse_loss.item()
            total_train_corr_loss += corr_loss.item()
        ...
        writer.add_scalar(
            "Loss/train_mse_loss", total_train_mse_loss / len(train_loader), epoch
        )
        writer.add_scalar(
            "Loss/train_corr_loss", total_train_corr_loss / len(train_loader), epoch
        )
