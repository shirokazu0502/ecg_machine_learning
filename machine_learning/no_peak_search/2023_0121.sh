...
# subject_group="2"
# #腹部につけた時のデータ
measure_names="matumoto yoshikura takahashi taniguchi kawai goto asano nakanishi gobara patient4 patient5 patient6 patient8 patient9"
measure_dates="1128 1130 1102 1107 1115 1219 0710 0717 0723 0722 0729 1001 1001 1001 1109 1109"
# measure_names="patient4 patient5 patient6 patient8 patient9"
# measure_dates="1001 1001 1001 1109 1109"
dataset_date="1109"
subject_group="1"
    ...
    for T_weight in $T_weights; do
        for R_weight in $R_weights; do
            for name in $measure_names; do
                python3 vae_goto_val_pt.py --TARGET_NAME "$name" --epochs 200 --latent_size 20 --beta 1 --mode train --transform_type normal --Dataset_name pqrst_nkmodule_since"$dataset_date"_cwt --loss_pt_on_off off --loss_pt_on_off_R_weight "$R_weight" --augumentation "$augumentation" --loss_pt_on_off_P_weight "$P_weight" --loss_pt_on_off_T_weight "$T_weight"
                python3 vae_goto_val_pt.py --TARGET_NAME "$name" --epochs 200 --latent_size 20 --beta 1 --mode zplot --transform_type normal --Dataset_name pqrst_nkmodule_since"$dataset_date"_cwt --loss_pt_on_off off --loss_pt_on_off_R_weight "$R_weight" --augumentation "$augumentation" --loss_pt_on_off_P_weight "$P_weight" --loss_pt_on_off_T_weight "$T_weight"
                python3 vae_goto_val_pt.py --TARGET_NAME "$name" --epochs 200 --latent_size 20 --beta 1 --mode test --transform_type normal --Dataset_name pqrst_nkmodule_since"$dataset_date"_cwt --loss_pt_on_off off --loss_pt_on_off_R_weight "$R_weight" --augumentation "$augumentation" --loss_pt_on_off_P_weight "$P_weight" --loss_pt_on_off_T_weight "$T_weight"
            done
