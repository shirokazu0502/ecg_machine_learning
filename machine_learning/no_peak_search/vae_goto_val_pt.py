...
def loss_fn_mse_PRT_QS(recon_x, x, mean, log_var, datalength, args):
...
def main(args):
    ...
    if args.mode == "train":
        ...
        for epoch in range(args.epochs):
            ...
            print("loader")
            for iteration, (x, xo, label_name) in enumerate(train_data_loader):
            ...
            test_val = []
            with torch.no_grad():
                for j, (x, xo, label_name) in enumerate(train_data_loader):
                    x, xo = x.to(device), xo.to(device)
                    recon_x, mean, log_var, z = vae(x)
                    for i in range(len(label_name)):
                        label_temps.append(label_name[i])
        ...
        output_file = os.path.join(
            OUTPUT_MAE_DIR
            + "/MAE_leave_1_out_{}_PRTweight_{}_aug{}_epoch{}_z{}.csv".format(
                args.Dataset_name,
                str(args.loss_pt_on_off_R_weight),
                args.augumentation,
                args.epochs,
