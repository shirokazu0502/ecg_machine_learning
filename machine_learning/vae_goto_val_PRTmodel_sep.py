...
def loss_fn_mse_PRT(
    ...
    Q_peaks = pt_index[:, 5].item()
    S_peaks = pt_index[:, 6].item()
    ...
    for i in range(batch_size):
        ...
        # print(index1)
        # print(index2)
        index1 = pt_index[:, 6].item() + 5  # q波ピークから5つ後ろをR_onsetとする
        index2 = pt_index[:, 7].item() - 5  # s波ピークから5つ前をR_offsetとする
...
def compute_pearson(xo, recon_x):
    # xoとrecon_xの形状を確認
    if xo.shape != recon_x.shape:
        raise ValueError("xoとrecon_xの形状が一致しません。")

    # ピアソン相関係数を計算
    pearson_corr = np.corrcoef(xo.flatten(), recon_x.flatten())[0, 1]
    return pearson_corr
...
def all_test_vae(test_loader, vae_dict, device, ts, ecg_ch_names, args):
    ...
    with torch.no_grad():
        for j, (x, xo, label_name, pt_index) in enumerate(test_loader):
            ...
            # after_r_index = 230
            # パラメータ設定
            p_end_original = pt_index[:, 2].item()  # P波の終わり（元々の区切り）
            r_start_original = pt_index[:, 6].item() + 5  # R波の始まり（元々の区切り）
            r_end_original = pt_index[:, 7].item() - 5  # R波の終わり（元々の区切り）
            t_start_original = pt_index[:, 3].item()  # T波の始まり（元々の区切り）
            overlap_duration_pr = (
...
class EarlyStopping:
    """
    検証用ロスの改善が見られなくなった場合に学習を早期終了させる。
    また、最も性能が良かったモデルの重みを保存する。
    """

    def __init__(self, patience=7, verbose=False, delta=0, path="checkpoint.pt"):
        """
        Args:
            patience (int): ロスが改善しなくなってから待つエポック数。
            verbose (bool): 改善が記録された際にメッセージを出力するかどうか。
            delta (float): 改善とみなすための最小変化量。
            path (str): 最良モデルの重みを保存するパス。
        """
    ...
    if args.mode == "train":
        ...
        for target_weight in weight_keys:
            ...
            for epoch in range(args.epochs):
                ...
                early_stopping(test_loss / len(test_loader), vae)
                # 早期終了のフラグが立った場合、学習ループを抜ける
        ...
        print("TEST MODE::\n")
        data_to_write = all_test_vae(
            test_loader, vae_dict, device, ts, ecg_ch_names, args
        )
        output_file = os.path.join(
            args.mae_folder
        ...
        all_test_vae(test_loader, vae_dict, ts, ecg_ch_names, args)
        # input()
        ...
        with torch.no_grad():
            for j, (x, xo, label_name, pt_index) in enumerate(test_loader):
                ...
                before_r_index = pt_index[:, 6].item() + 5
                after_r_index = pt_index[:, 7].item() - 5
                recon_x = recon_x_p.view(-1, ecg_ch, datalength)
                # before_r_indexより前のデータはrecon_x_pのデータを活用
...
if __name__ == "__main__":
    current_time = "0708_add_pr_st_height"
