...
def Dataset_setup_8ch_pt_augmentation(
    TARGET_NAME, transform_type, Dataset_name, dataset_num, DataAugumentation, lookback
):
    datalength = 400
    ecg_ch_num = 8
    Data = []
    ...
    for j in range(len(Train_list)):
        ...
        for i in range(dataset_num):
            ...
            if not os.path.isfile(path) or not os.path.isfile(pt_path):
                ...
                # スライディングウィンドウでlookbackの長さを考慮してデータセットを作成
                for start in range(0, PGV_train.size(2) - lookback):
                    input_seq = PGV_train[
                        :, :, start : start + lookback
                    ]  # lookback長の入力シーケンス
                    target_seq = ECG_train[
                        :, :, start + lookback
                    ]  # 次の時刻のECGデータ（予測するターゲット）

                    PGV_train_set.append(input_seq)  # (lookback, 15)
                    ECG_train_set.append(target_seq)  # (8,)
                # time.sleep(30)
    ...
    for j in range(len(Test_list)):
        ...
        for i in range(dataset_num):
            ...
            if not os.path.isfile(path):
                ...
                for start in range(0, PGV_test.size(2) - lookback):
                    input_seq = PGV_test[
                        :, :, start : start + lookback
                    ]  # lookback長の入力シーケンス
                    target_seq = ECG_test[
                    ...
                    PGV_test_set.append(input_seq)  # (lookback, 15)
                    ECG_test_set.append(target_seq)  # (8,)

                print(len(PGV_test), PGV_test.shape)
                print(len(PGV_test_set), len(ECG_test_set))
                print(PGV_test_set[0].shape, ECG_test_set[0].shape)
                # time.sleep(30)
    ...
    print(len(PGV_train_set), len(ECG_train_set))
    print(PGV_train_set[0].shape, ECG_train_set[0].shape)
