...
writer_P = SummaryWriter(log_dir="runs/P_log")
writer_R = SummaryWriter(log_dir="runs/R_log")
writer_T = SummaryWriter(log_dir="runs/T_log")
# 上記を辞書にまとめる
writer_dict = {
    "P": writer_P,
    "R": writer_R,
...
def train_unet(model, train_loader, optimizer, criterion, epochs, device):
    model.train()
...
def main(args):
    ...
    if args.mode == "train":
        ...
        for target_weight in weight_keys:
            ...
            for epoch in range(args.epochs):
                ...
                for iteration, (x, xo, label_name, pt_index) in enumerate(
                    ...
                    if iteration % args.print_every == 0:
                        ...
                        if args.conditional:
                            ...
                            ifx = unet.inference(n=c.size(0), c=c)
                        else:
                            ifx = unet.inference(n=15)
                ...
                writer_dict[target_weight].add_scalar(
                    "Loss/train_loss", loss_keep, epoch
                )
                writer_dict[target_weight].add_scalar("Loss/val_loss", acc_keep, epoch)
                writer_dict[target_weight].add_scalar(
                    "KL Divergence",
                    kdl_keep,
                    epoch,
            ...
            if target_weight == "P":
                ...
                torch.save(
                    unet.state_dict(),
                    os.path.join("model_pth", "unet_rwave_weight.pth"),
                )
                torch.save(
                    unet.state_dict(),
                ...
                torch.save(
                    unet.state_dict(),
                    os.path.join("model_pth", "unet_twave_weight.pth"),
                )
                torch.save(
                    unet.state_dict(),
                    os.path.join(args.fig_root, str(ts), "unet_twave_weight.pth"),
                )
        ...
        print(test_loader)
        # input()
        # P波、R波、T波のunetをそれぞれ読み込む
        unet_dict["P"].load_state_dict(
            torch.load(
                os.path.join("model_pth", "unet_pwave_weight.pth"),
                map_location=lambda storage, loc: storage,
            )
        )
        unet_dict["R"].load_state_dict(
            torch.load(
                os.path.join("model_pth", "unet_rwave_weight.pth"),
                map_location=lambda storage, loc: storage,
            )
        )
        unet_dict["T"].load_state_dict(
            torch.load(
                os.path.join("model_pth", "unet_twave_weight.pth"),
                map_location=lambda storage, loc: storage,
            )
...
if __name__ == "__main__":
    current_time = "0507_1120_z8_ch15*4"
