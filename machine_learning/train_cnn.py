...
def train_cnn(
    ...
    for epoch in range(epochs):
        ...
        model.train()
        for x, xo, _, pt_index in train_loader:
            x, xo, pt_index = x.to(device), xo.to(device), pt_index.to(device)
            optimizer.zero_grad()
            outputs = model(x)
...
def test_cnn(model, test_loader, device, args, exp_dir):
    ...
    pearson_all_mean_score = np.mean(all_pearson_scores)
    ...
    output_file = os.path.join(
        args.mae_folder,
        f"MAE_leave_1_out_{args.Dataset_name}_{args.model_type}.csv",
    )
...
def main():
    ...
    if args.mode == "train":
        ...
        criterion = utils.loss_fn_weighted_mse_and_corr  # Using U-Net loss as a default
