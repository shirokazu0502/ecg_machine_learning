...
from config.settings import DATA_DIR, BASE_DIR, PROCESSED_DATA_DIR, OUTPUT_DIR, RAW_DATA_DIR, TEST_DIR, RATE,RATE_15CH, TIME, DATASET_MADE_DATE, OUTPUT_MAE_DIR
#=====================
cmap = 'tab10'
...
def plot_fig(numplotfig,recon_x,xo,datalength,ts,args,label_name,ecg_ch_names):
    ...
    for p in range(numplotfig):
        ...
        for q in range(ecg_ch):
            ...
            plt.legend(["predict","ECG"],bbox_to_anchor=(0.60,1),loc="upper left",fontsize=15,framealpha=1.0)
...
def plot_fig_train(recon_x,xo,datalength,ts,args,iteration_num,batch_size_num,epoch_num):
    sample_rate=500
    sample_num=750
    xticks=np.linspace(0.0,1.0/sample_rate*sample_num,sample_num)
    ecg_ch=args.ecg_ch_num
    for p in range(batch_size_num):
        ...
        for q in range(ecg_ch):
            ...
            #plt.plot(x[p].cpu().data.numpy()[0:datalength], color="green", linewidth=1.0, linestyle="-")
            #plt.plot(min_max_old(recon_x2[p][q].cpu().data.numpy()), color="red", linewidth=1.0, linestyle="-")
            plt.plot(xticks,recon_x2[p].cpu().data.numpy(), color="red", linewidth=1.0, linestyle="-")
            #plt.plot(xo[p].cpu().data.numpy(), color="blue", linewidth=1.0, linestyle="-")#xoが（３２，４００）かたちになってる、
            plt.plot(xticks,xo2[p].cpu().data.numpy(), color="blue", linewidth=1.0, linestyle="-")#xoが（３２，４００）かたちになってる、
            #plt.subplots_adjust(left=0.125,bottom=0.1,right=0.9,top=0.9,wspace=0.2,hspace=0.35)
            # plt.ylim(0,1)
            plt.xlabel("second")
            plt.axis('on')
            ...
            if not os.path.exists(os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num))):
                os.mkdir(os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num)))

            plt.savefig(
                os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num),"train_x_xo_{:d}_ch{:d}.png".format(p,q)),
                dpi=300,
                )
            # plt.show()
            plt.cla()
...
def plot_fig_train_name(recon_x,xo,datalength,ts,args,iteration_num,batch_size_num,epoch_num,label_name):
    sample_rate=500
    ...
    # for p in range(2):
        ...
        for q in range(ecg_ch):
            ...
            plt.savefig(
                ...
                os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num),"ch{:d}_train_x_xo_{}.png".format(q,label_name[p])),
                dpi=300,
                )
            # plt.show()
            plt.cla()
...
def plot_fig_15ch_only(recon_x,datalength,ts,args,batch_size_num,label_name,pt_index):
    sample_rate=500
    # sample_num=750
    sample_num=args.datalength
    dt=1/sample_rate
    ...
    # for p in range(2):
        for q in range(ecg_ch):
            ...
            plt.grid(which="both", axis ="x", alpha=0.8,linestyle ="--",linewidth=1)
            # plt.legend(["predict","ECG"],bbox_to_anchor=(1.05,1),loc="upper left")
            plt.legend(["predict","ECG","P_onset","T_offset"],bbox_to_anchor=(1.05,1),loc="upper left")
            ...
            plt.savefig(
                # os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num),"ch{:d}_train_x_xo_{:d}.png".format(q,p)),
                os.path.join(args.fig_root, str(ts),"ch={}_15ch_only_test_x_xo_{}.png".format(ecg_ch_names[q],label_name[p])),
                dpi=300,
                )
            # plt.show()
            plt.cla()
...
def plot_fig_test_name_8ch_2row(recon_x,xo,datalength,ts,args,batch_size_num,label_name,acc,pt_index,ecg_ch_names):
    sample_rate=500
    # sample_num=750
    sample_num=args.datalength
    dt=1/sample_rate
    ...
    # for p in range(2):
        ...
        plt.savefig(
            ...
            os.path.join(args.fig_root, str(ts),"plot_all_channel_8ch_2row_{}test_x_xo.SVG".format(label_name[p])),
            dpi=300,
            )
        # plt.show()
        plt.cla()
        plt.clf()
        plt.close()
def plot_fig_test_name_8ch(recon_x,xo,datalength,ts,args,batch_size_num,label_name,acc,pt_index,ecg_ch_names):
    sample_rate=500
    # sample_num=750
    sample_num=args.datalength
    dt=1/sample_rate
...
def plot_fig_test_name(recon_x,xo,datalength,ts,args,batch_size_num,label_name,acc,pt_index,ecg_ch_names):
    sample_rate=500
    # sample_num=750
    sample_num=args.datalength
    dt=1/sample_rate
    ...
    # for p in range(2):
        ...
        for q in range(ecg_ch):
            ...
            # plt.legend(["predict","ECG"],bbox_to_anchor=(1.05,1),loc="upper left")
            plt.legend(["predict","ECG","P_onset","T_offset"],bbox_to_anchor=(1.05,1),loc="upper left")
            plt.title(label_name[p]+'_acc={}_ch={}'.format(str(acc[p][q]),ecg_ch_names[q]))
            plt.tight_layout()
            ...
            plt.savefig(
                # os.path.join(args.fig_root, str(ts),"epoch{:d}_iteration{:d}".format(epoch_num,iteration_num),"ch{:d}_train_x_xo_{:d}.png".format(q,p)),
                os.path.join(args.fig_root, str(ts),"ch={}_test_x_xo_{}_MAE={}.png".format(ecg_ch_names[q],label_name[p],str(acc[p][q]))),
...
def loss_fn_bce(recon_x, x, mean, log_var,datalength):
   ...
   # print(recon_x)
    ...
    BCE = torch.nn.functional.binary_cross_entropy(
        ...
        recon_x.view(-1, datalength), x.view(-1, datalength), reduction='sum') #二値交差エントロピー損失、この関数内のxはxoのことつまり予測値
...
def loss_fn_mse_pt(recon_x, x, mean, log_var,datalength,pt_index,args):
    ...
    MSE=torch.nn.MSELoss(reduction="none")#x,recon_xと全く同じ形状で(y_n-y'_n)^2の計算して要素に並べる。
    MSE_loss=MSE(recon_x.view(-1,ecg_ch_num,datalength), x.view(-1,ecg_ch_num,datalength))
    num=0
    MSE_loss_temp=0
    for i in range(batch_size):
        MSE_loss_temp+=torch.mean(MSE_loss[i,:,pt_index[i][0]:pt_index[i][1]])#ミニバッチ内のデータそれぞれについてP波の始まりとT波の終わりまでの範囲でMSE計算
    ...
    beta=args.beta
    # KLD=beta*KLD
    return (alpha*MSE_loss_pt + beta*KLD)/ batch_size, MSE_loss_pt, KLD / batch_size
...
def loss_fn_mse(recon_x, x, mean, log_var,datalength,args):
    ...
    MSE_loss=MSE(recon_x.view(-1, datalength), x.view(-1, datalength))#ミニバッチ内全てで平均。ミニバッチ内のそれぞれのデータの形状が同じ
    KLD = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
    alpha=args.alpha
    beta=args.beta
    # print(MSE_loss)
    ...
    return alpha*MSE_loss + beta*KLD / batch_size, MSE_loss/batch_size, KLD/batch_size #MSEはreduction=meanで既にバッチで割られているからここではバッチ数で割らない
...
def loss_fn_mse_PRT_QS(recon_x, x, mean, log_var,datalength,args,pt_index):
    ...
    for i in range(batch_size):
        ...
        MSE_loss_keep=MSE_loss_1*weight_P+MSE_loss_2*weight_T+MSE_loss_R*weight_R#（インデックス長さ×チャネル数）で割る。まだバッチの数では割れていない。
    ...
    return (alpha*MSE_loss + beta*KLD / batch_size), alpha*MSE_loss, beta*KLD/batch_size
    # return (beta*KLD) / batch_size, MSE_loss/batch_size, KLD/batch_size
...
def plot_scatter_2d_pos(z,labels,latent_size,ts,fig_num=1, figsize=(4, 3), dpi=300, facecolor='w', edgecolor='k'):
    ...
    for i in range(z.shape[0]):
        ...
        if label not in legend_dict:
            legend_dict[label] = ax.scatter(z[i,0], z[i,1], s=40,label=label,color=color)
    ...
    ax.legend(handles=legend_elements, title='Labels', fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    # plt.show()
def plot_scatter_2d(z,labels,latent_size,ts,args,fig_num=1, figsize=(4, 3), dpi=300, facecolor='w', edgecolor='k'):
    if latent_size == 2:
        z = np.array(z)
    else:
    ...
    for i in range(z.shape[0]):
        ...
        color = colors(unique_labels.tolist().index(label) % num_labels)
        if label not in legend_dict:
            legend_dict[label] = ax.scatter(z[i,0], z[i,1], s=10,label=label,color=color)
    ...
    # plt.legend(handles=legend_elements, title='Labels',fontsize=8)
    ax.legend(handles=legend_elements, title='Labels', fontsize=8, bbox_to_anchor=(1.05, 1), loc='upper left')
    plt.title("epoch={},beta={},latent_size={},dim_red={},".format(args.epochs,args.beta,args.latent_size,args.dim_red))
    # cbar = fig.colorbar(sc, aspect=30)
    # cbar.ax.tick_params(direction='out')
    # cbar.ax.yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    # plt.show()
...
class MAE(torch.nn.Module):#MSEの個々の損失の平方根はMAEと同じなる。reduction=Noneの時だけ
...
class MAE_2(torch.nn.Module):#上のMAEと書き方は違うけど同じ値になった。こっちのが一般的な書き方。
    def __init__(self, reduction='none'):
        super().__init__()
...
def save_csv2(data,ts,args,label_name,data_rec_or_xo):
    ...
    output_path = os.path.join(OUTPUT_DIR+"/{}/PRTweight_{}_{}_{}_augumentation={}/Waveforms".format(args.Dataset_name,str(args.loss_pt_on_off_P_weight),str(args.loss_pt_on_off_R_weight),str(args.loss_pt_on_off_T_weight),args.augumentation),"{}".format(args.TARGET_NAME))
    ...
    for p in range(batch_size):
        if(data_rec_or_xo=='recon_x'):
            ...
            output_file = os.path.join(output_path,"{}_reconx.csv".format(label_name[p]))
        else:
...
def save_csv(data,ts,args,label_name,data_rec_or_xo):
    ...
    # output_path = os.path.join(args.fig_root, str(ts),"waveforms")
    # output_path = os.path.join("leave_1_out_{}_R_weight_{}_augumentation={}".format(args.Dataset_name,str(args.loss_pt_on_off_R_weight),args.augumentation),"waveforms","{}".format(args.TARGET_NAME))
    if args.loss_pt_on_off_R_weight!=None and args.loss_pt_on_off_P_weight!=None and args.loss_pt_on_off_T_weight!=None:
        output_path = os.path.join("leave_1_out_{}_PRTweight_{}_{}_{}_augumentation={}".format(args.Dataset_name,str(args.loss_pt_on_off_P_weight),str(args.loss_pt_on_off_R_weight),str(args.loss_pt_on_off_T_weight),args.augumentation),"waveforms","{}".format(args.TARGET_NAME))
    else:
        output_path = os.path.join("leave_1_out_{}_R_weight_{}_augumentation={}".format(args.Dataset_name,str(args.loss_pt_on_off_R_weight),args.augumentation),"waveforms","{}".format(args.TARGET_NAME))
    os.makedirs(output_path, exist_ok=True)
...
#==============================================goto=======================
    ...
    #train_dataset,test_dataset=Dataset.Dataset_setup_new(args.target)
    # train_dataset,test_dataset=Dataset.Dataset_setup_new_multiset(args.target,args.target_set)
    # train_dataset,test_dataset=Dataset.Dataset_setup_8ch(args.target,args.target_set)
    # train_dataset,test_dataset=Dataset.Dataset_setup_12ch(args.target,args.target_set,args.TARGET_NAME)

    # train_dataset,test_dataset=Dataset.Dataset_setup_12ch(TARGET_NAME=args.TARGET_NAME,transform_type=args.transform_type,Dataset_name=args.Dataset_name)
    # train_dataset,test_dataset=Dataset.Dataset_setup_12ch_pt(TARGET_NAME=args.TARGET_NAME,transform_type=args.transform_type,Dataset_name=args.Dataset_name,dataset_num=args.dataset_num)
    # train_dataset,test_dataset=Dataset.Dataset_setup_8ch_pt(TARGET_NAME=args.TARGET_NAME,transform_type=args.transform_type,Dataset_name=args.Dataset_name,dataset_num=args.dataset_num)
    train_dataset,test_dataset=Dataset.Dataset_setup_8ch_pt_augmentation(TARGET_NAME=args.TARGET_NAME,transform_type=args.transform_type,Dataset_name=args.Dataset_name,dataset_num=args.dataset_num,DataAugumentation=args.augumentation)
    # input("")
...
#==================================================================
    ...
    ts = datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d_%H%M%S')+'_Dataset_name='+args.Dataset_name+'_transform_type='+args.transform_type+'_'+args.mode+'_TARGETNAME='+str(args.TARGET_NAME)+'_beta='+str(args.beta)+'_alpha='+str(args.alpha)+'_loss_pt_on_off='+args.loss_pt_on_off+"PRT="+PRT_weight+'_augument='+args.augumentation
    ...
    ecg_ch=args.ecg_ch_num
    if(ecg_ch==12):
        ecg_ch_names=["Ⅰ","Ⅱ","A3","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6"]
    if(ecg_ch==8):
    ...
    if args.mode == "train":
        ...
        for epoch in range(args.epochs):
            ...
            for iteration, (x, xo,label_name,pt_index) in enumerate(train_data_loader):
                x, xo = x.to(device), xo.to(device)
                x_check = torch.reshape(x, (-1, 15, args.datalength))
                # print(pt_index)
                # input("")
                # print(x_check[0][0].cpu().numpy())
                # plt.plot(x_check[0][0].cpu().numpy())
                # plt.show()
                # print(x.shape)
                # input()
                #if(iteration==0 ):
                #    # print(x.shape)
                #    # print(x)
                #    xx = torch.reshape(x.detach(), (-1,15, 384))
                #    # print(xx.shape)
                #    df_x, df_xo = pd.DataFrame(xx[0].to("cpu")), pd.DataFrame(xo.detach().to("cpu"))
                #    df_x.T.to_csv(os.path.join(args.fig_root, str(ts),"multi_"+str(args.target_set))+"_"+str(args.target)+"{:d}".format(epoch)+".csv",index=False,header=False)#毎回違うのが入っていることを確認
                ...
                if args.loss_fn_type=="bce":
                    loss,bce,kdl = loss_fn_bce(recon_x, xo, mean, log_var,datalength)
                if args.loss_fn_type=="mse":
                    if args.loss_pt_on_off=='off':
                        ...
                        if args.loss_pt_on_off_R_weight!=None and args.loss_pt_on_off_P_weight!=None and args.loss_pt_on_off_T_weight!=None:
                            # loss,mse,kdl = loss_fn_mse_PRT_weight(recon_x, xo, mean, log_var,datalength,args)#0.8秒間ｓのデータ全体で損失計算
                            # loss_org,mseaa,kdlaaaa = loss_fn_mse_PRT_weight(recon_x, xo, mean, log_var,datalength,args)#0.8秒間ｓのデータ全体で損失計算
                            # loss,mse,kdl = loss_fn_mse_PRT_weight_QS(recon_x, xo, mean, log_var,datalength,args,pt_index)#0.8秒間ｓのデータ全体で損失計算,ただしQRSに重みをつける
                            loss,mse,kdl = loss_fn_mse_PRT_QS(recon_x, xo, mean, log_var,datalength,args,pt_index)#0.8秒間ｓのデータ全体で損失計算,ただしQRSに重みをつける
                            ...
                            loss,mse,kdl = loss_fn_mse(recon_x, xo, mean, log_var,datalength,args)#0.8秒間ｓのデータ全体で損失計算

                    if args.loss_pt_on_off=='on':
                        loss,mse,kdl = loss_fn_mse_pt(recon_x, xo, mean, log_var,datalength,pt_index=pt_index, args=args)
                ...
                if iteration % args.print_every == 0:
                    print("Epoch {:02d}/{:02d} Batch {:04d}/{:d}, Loss {:9.4f},Acc(mse){:9.4f}".format(
                        epoch, args.epochs, iteration, len(train_data_loader)-1, loss_keep.item()/(iteration+1),acc_keep.item()/(iteration+1)))
            ...
            with torch.no_grad():
                ...
                for j,(x,xo,label_name,pt_index) in enumerate(test_loader):
                    x, xo = x.to(device), xo.to(device)
                    # print(x.shape)
                    # xx = torch.reshape(x, (-1,15, args.datalength))
                    # xxo = torch.reshape(xo, (-1,12, args.datalength))
                    # df_x, df_xo = pd.DataFrame(xx[0].to("cpu")), pd.DataFrame(xxo.to("cpu"))
                    # df_x.T.to_csv(os.path.join(args.fig_root, str(ts),"multi_"+str(args.target_set))+"_"+str(args.target)+".csv",index=False,header=False)
                    #print(x.to(device).size(), model(x.to(device)).size(), t.to(device).size())
                    ...
                    for i in range(len(label_name)):
                        label_temps.append(label_name[i])
                    # print(label_temps)
                   #loss = loss_fn(recon_x, xo, mean, log_var)
                   #loss_test += loss.item()
                    # rmse_loss=RMSELoss(reduction="none")
                    # mae_loss=MAE(reduction="none")
                    ...
                    if args.loss_pt_on_off=='off':
                        ...
                        test_val=test_val+cul_val_no_pt(acc_mae)#cul_val_no_ptは波形全体でrmse計算
                    else:
                        # test_val=test_val+cul_val(pt_index,acc_rmse)#cul_valはptの範囲でRMSE計算
                        test_val=test_val+cul_val(pt_index,acc_mae)#cul_valはptの範囲でmae計算
                    # acc_rmse=rmse_loss(recon_x.view(-1,ecg_ch, datalength), xo.view(-1,ecg_ch, datalength))
                    # acc_rmse_per_batch = torch.sum(acc_rmse,dim=(1,2))
                    # acc_rmse_per_batch = acc_rmse_per_batch.to('cpu').detach().numpy().copy()
                    # print(acc_rmse_per_batch.shape)
                    # acc_temp.append(acc_rmse_per_batch)
            # print(test_val)
            ...
            test_acc_temps.append(test_val_ndarray.mean())
                ...
                # test_acc_all=np.concatenate(acc_temp,axis=0)
                # print(test_acc_all)
                # test_acc_mean=(np.mean(test_acc_all))
                # print("np.sum(acc_all)"+str(test_acc_mean))
                # test_acc_temps.append(test_acc_mean)


            logs['loss'].append(loss_keep.item()/(iteration+1))
            if args.loss_fn_type=="bce":
                logs['bce'].append(bce_keep.item()/(iteration+1))
            if args.loss_fn_type=="mse":
                logs['mse'].append(mse_keep.item()/(iteration+1))
            logs['kdl'].append(kdl_keep.item()/(iteration+1))
            logs['acc'].append(acc_keep.item()/(iteration+1))

        # print(test_acc_temps)
        "テストデータのMAE推移プロット"
        plt.xlabel("epoch")
        plt.ylabel("MAE")
        ...
        plt.title("MAE values of test data epoch 0-{}".format(epoch))
        ...
        # plt.xlabel("epoch")
        # plt.ylabel("acc")
        # plt.plot(logs["acc"],color="red")
        # plt.legend(["train_acc"])
        # plt.title("RMSE values of test data epoch 0-{}".format(epoch))
        # plt.savefig(os.path.join(args.fig_root, str(ts),"acc"),dpi=300)
        # # plt.show()
        # plt.close()
        ...
        if args.loss_fn_type=="bce":
            ...
            plt.savefig(os.path.join(args.fig_root, str(ts),"loss_epoch_all.png"),dpi=300)
            # plt.show()
            plt.close()
            ...
            #plt.plot(logs["kdl"],color="green")
            #plt.legend(["loss_all","mse","kdl"])
            plt.legend(["loss"])
            plt.savefig(os.path.join(args.fig_root, str(ts),"loss_epoch_epoch.png"),dpi=300)
        #    plt.show()
       ...
       # #モデル保存
        ...
        #print("encorder_conv_layer0")
        #print(vae.encoder.MLP.AC0.weight)
        #print(vae.encoder.MLP_1.AC0.bias)
        #print("===========================")
        #print("encorder_conv_layer_BN")
        #print(vae.encoder.MLP.AB0.weight)
        #print(vae.encoder.MLP.AB0.bias)
        #print(vae.encoder.MLP.AB0.running_mean)
        #print(vae.encoder.MLP.AB0.running_var)
        #print("===========================")
        #print("encorder_Linear0")
        #print(vae.encoder.MLP.L0.weight)
        #print(vae.encoder.MLP.L0.bias)
        #print("===========================")
        #print("encorder_Linear1")
        #print(vae.encoder.MLP.L1.weight)
        #print(vae.encoder.MLP.L1.bias)
        #print("===========================")
        print("===========================")
        print("encorder_Linear1")
        ...
        with torch.no_grad():
            for j,(x,xo,label_name,pt_index) in enumerate(test_loader):
               ...
               #loss_test += loss.item()
                ...
                # print("koko2")
                print(label_name)
                # plot_fig(numplotfig,recon_x,xo,datalength,ts,args,label_name,ecg_ch_names=ecg_ch_names)
                ...
                mae_loss=MAE_2(reduction="none")#RMSELoss関数を使っているけどreducutionがnoneやから
                ...
                # acc_rmse=rmse_loss(recon_x,xo)
                acc_mae=mae_loss(recon_x,xo)
                # acc_mae_2=mae_loss_2(recon_x,xo)
                # print(acc_mae)
                # print(acc_mae_2)
                # input("")
                ...
                # test_val = np.array([array_item.tolist() for array_item in test_val], dtype=np.float32)
                # print(test_val)
                test_val_all=test_val_all+test_val.tolist()
                # print(test_val_all)

                if args.loss_pt_on_off=='off':#精度評価でptの範囲だけを計算するかどうか。offで０．８秒間全体を評価計算する。
                    # test_val_12ch=cul_val_per_12ch_no_pt(acc_rmse)
                    test_val_12ch=cul_val_per_12ch_no_pt(acc_mae)
                else:
                    # test_val_12ch=cul_val_per_12ch(pt_index,acc_rmse)
                    test_val_12ch=cul_val_per_12ch(pt_index,acc_mae)

                test_val_12ch = np.array([array_item.tolist() for array_item in test_val_12ch], dtype=np.float32)
                # test_val_12ch = test_val_12ch.reshape(-1, 12)
                test_val_12ch = test_val_12ch.reshape(-1, args.ecg_ch_num)
                test_val_12ch_all=test_val_12ch_all+test_val_12ch.tolist()

                # print("test_val_12ch")
                # print(test_val_12ch)
                # print(len(test_val_12ch_all))
                # acc_rmse_per_batch =test_val
                acc_rmae_per_batch =test_val
                # acc_rmse_per_batch_ch=test_val_12ch


                # acc_rmse_per_batch=torch.sum(acc_rmse,dim=(1,2))
                # acc_rmse_per_batch = acc_rmse_per_batch.to('cpu').detach().numpy().copy()
                # acc_rmse_per_batch_ch=torch.sum(acc_rmse,dim=2)
                # acc_rmse_per_batch_ch = acc_rmse_per_batch_ch.to('cpu').detach().numpy().copy()



                # acc=mse_loss(recon_x.view(-1, datalength), xo.view(-1, datalength))
                # li1=recon_x.to('cpu').view(-1,datalength).detach().numpy()
                # li2=xo.to('cpu').view(-1,datalength).detach().numpy()
                # li1_r=li1.reshape([-1,datalength])
                # li2_r=li2.reshape([-1,datalength])
                # s1=pd.Series(li1_r)
                # s2=pd.Series(li2_r)
                # print(s1)
                # print(s2)
                ...
                #     plt.close()
                   ...
                   #recon_xf=fourier(recon_x)
                   #xof=fourier(xo)
                   #plot_fig(numplotfig,recon_xf,xof,datalength,ts,args)
                # print(j)
                # if j==0:
                # print("z")
                # print(z)
                ...
                #     # input("")
                #     plot_fig_test_name_8ch(recon_x=recon_x,xo=xo,datalength=datalength,ts=ts,args=args,batch_size_num=batch_size_now,label_name=label_name,acc=test_val_12ch,pt_index=pt_index,ecg_ch_names=ecg_ch_names)
                ...
                # input("")
                # plot_fig_test_name_8ch(recon_x=recon_x,xo=xo,datalength=datalength,ts=ts,args=args,batch_size_num=batch_size_now,label_name=label_name,acc=test_val_12ch,pt_index=pt_index,ecg_ch_names=ecg_ch_names)
                plot_fig_test_name_8ch_2row(recon_x=recon_x,xo=xo,datalength=datalength,ts=ts,args=args,batch_size_num=batch_size_now,label_name=label_name,acc=test_val_12ch,pt_index=pt_index,ecg_ch_names=ecg_ch_names)
                # plot_fig_test_name(recon_x=recon_x,xo=xo,datalength=datalength,ts=ts,args=args,batch_size_num=batch_size_now,label_name=label_name,acc=test_val_12ch,pt_index=pt_index,ecg_ch_names=ecg_ch_names)
                # output_path=save_csv(data=recon_x,args=args,ts=ts,label_name=label_name,data_rec_or_xo='recon_x')
                # output_path=save_csv(data=xo,args=args,ts=ts,label_name=label_name,data_rec_or_xo='xo')
                output_path=save_csv2(data=recon_x,args=args,ts=ts,label_name=label_name,data_rec_or_xo='recon_x')
                output_path=save_csv2(data=xo,args=args,ts=ts,label_name=label_name,data_rec_or_xo='xo')
                # plot_fig_train(recon_x=recon_x,xo=xo,datalength=datalength,ts=ts,args=args,iteration_num=iteration,batch_size_num=batch_size_now,epoch_num=epoch)
            # plot_scatter_2d(z=z_temps,labels=label_temps,latent_size=latent_size,ts=ts,args=args)
        ...
        data_to_write={"TARGET_NAME":args.TARGET_NAME,
        ...
        output_file = os.path.join(OUTPUT_MAE_DIR+"/MAE_leave_1_out_{}_PRTweight_{}_{}_{}_augumentation={}.csv".format(args.Dataset_name,str(args.loss_pt_on_off_P_weight),str(args.loss_pt_on_off_R_weight),str(args.loss_pt_on_off_T_weight),args.augumentation))
        write_to_csv(output_file,data=data_to_write)

    elif args.mode == "15ch_only":
        print("15ch_only MODE::\n")
        test_dataset=Dataset.Dataset_setup_12ch_pt_15ch_only(TARGET_NAME="osaka",transform_type=args.transform_type,Dataset_name="15ch_only",dataset_num=3)
        test_loader = DataLoader(test_dataset, batch_size=4, shuffle=False)
        print(len(test_dataset))
        # input()
        ...
        with torch.no_grad():
            for j,(x,label_name,pt_index) in enumerate(test_loader):
                ...
                print(batch_size_now)
                plot_fig_15ch_only(recon_x=recon_x,datalength=datalength,ts=ts,args=args,label_name=label_name,batch_size_num=batch_size_now,pt_index=pt_index)
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--target_set",type=int,default=1)
    # parser.add_argument("--Dataset_name",type=str,default="new_sensor_N=6_pt_8s")
    # parser.add_argument("--augumentation",type=str,default="height")
    # parser.add_argument("--augumentation",type=str,default="rt_and_height")
    # parser.add_argument("--augumentation",type=str,default="rt")
    parser.add_argument("--augumentation",type=str,default="")
    # parser.add_argument("--Dataset_name",type=str,default="pqrst_nkmodule")
    parser.add_argument("--Dataset_name",type=str,default="")
    parser.add_argument("--loss_pt_on_off",type=str,default="off")
    # parser.add_argument("--loss_pt_on_off_without_wave",type=str,default="R_1_100")
    ...
    # parser.add_argument("--dataset_num",type=int,default=15)
    ...
    # parser.add_argument("--train_batch_size", type=int, default=4) #default=256
    # parser.add_argument("--train_batch_size", type=int, default=8) #default=256
    parser.add_argument("--val_batch_size", type=int, default=1) #default=256
    parser.add_argument("--learning_rate", type=float, default=0.001)

    # parser.add_argument("--datalength", type=int, default=750)
    parser.add_argument("--datalength", type=int, default=400)
    #parser.add_argument("--datalength", type=int, default=250)
    ...
    #parser.add_argument("--enc_convlayer_sizes", type=list, default=[[15, 1], [30, 2],[60, 2],[120,2],[240,2]])#[[入力,ストライド],]
    parser.add_argument("--enc_convlayer_sizes", type=list, default=[[15, 1], [30, 2]])#[[入力,ストライド],]
    parser.add_argument("--enc_fclayer_sizes", type=list, default=[6000, 500, 64]) #一番目の適切な値は畳み込み層次第　エラーメッセージをみて調整するのが早い
    # parser.add_argument("--enc_fclayer_sizes", type=list, default=[11250, 500, 64]) #一番目の適切な値は畳み込み層次第　エラーメッセージをみて調整するのが早い
    #parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 500])
    #parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 800])
    # parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 9000])
    # parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 4800])#12chのとき
    parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 3200])#8chのとき
    # parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 512, 9000])
    # parser.add_argument("--dec_fclayer_sizes", type=list, default=[64, 216, 384])
    ...
    parser.add_argument("--dec_convlayer_sizes", type=list, default=[[16,2],[8,1]]) #通常畳み込み層が続くが、出力が単純（三角波）なので畳み込み層があってもなくても結果がそこまで変わらなかったらしい
    ...
    #parser.add_argument("--fig_root", type=str, default='figs_cross_v2') #学習過程及びテスト結果を出力するフォルダ
    # parser.add_argument("--fig_root", type=str, default='figs_newref') #学習過程及びテスト結果を出力するフォルダ
    parser.add_argument("--fig_root", type=str, default=OUTPUT_DIR+'/'+'figs_newref') #学習過程及びテスト結果を出力するフォルダ
    parser.add_argument("--conditional", action='store_true')
    parser.add_argument("--train_off", action='store_false')

    parser.add_argument("--pth", type=str, default=r'vae_cross_v2_5.pth') #学習モデルのファイル名指定
    parser.add_argument("--mode", type=str, default='train')
    parser.add_argument("--loss_fn_type", type=str, default='mse')#ECGの正規化なしだと上手くいった。
    parser.add_argument("--beta", type=int, default=1)
    parser.add_argument("--alpha", type=int, default=300000)
    parser.add_argument("--dim_red", type=str, default='')
    # parser.add_argument("--transform_type", type=str, default='random')
