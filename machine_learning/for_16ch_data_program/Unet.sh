...
p_augumentation="" #PR部分の延長短縮と高さを変えるデータ拡張を行う。
...
# for name in $measure_names; do
#     python3 unet_goto_val.py --TARGET_NAME "$name" --epochs 500 --beta 1 --mode train --transform_type normal --Dataset_name for_best_resample --loss_pt_on_off off --ave_data_flg $ave_data_flg
#     python3 unet_goto_val.py --TARGET_NAME "$name" --epochs 500 --beta 1 --mode test --transform_type normal --Dataset_name for_best_resample --loss_pt_on_off off  --ave_data_flg $ave_data_flg
# done
