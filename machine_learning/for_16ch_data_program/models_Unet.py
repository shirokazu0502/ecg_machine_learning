...
class UNet1D(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1):
        ...
        # --- エンコーダ ---
        # [cite_start]U-Netのエンコーダ（収縮パス）は4つのダウンサンプリングブロックで構成 [cite: 172]
        ...
        # --- ボトルネック (BiLSTM) ---
        # [cite_start]ダウンサンプリング処理にBiLSTM層を追加 [cite: 177]
        self.bilstm = nn.LSTM(
            input_size=512,
            hidden_size=512,
            num_layers=1,
        ...
        # --- デコーダ ---
        # [cite_start]U-Netのデコーダ（拡張パス）は4つのアップサンプリングブロックで構成 [cite: 172]
        self.up1 = nn.ConvTranspose1d(1024, 512, kernel_size=4, stride=2, padding=1)
        self.dec1 = DoubleConv(1024, 512, dropout=True)  # 512(up) + 512(skip)

        self.up2 = nn.ConvTranspose1d(512, 256, kernel_size=4, stride=2, padding=1)
        self.dec2 = DoubleConv(768, 256, dropout=True)  # 256(up) + 512(skip)

        self.up3 = nn.ConvTranspose1d(256, 128, kernel_size=4, stride=2, padding=1)
        self.dec3 = DoubleConv(384, 128, dropout=True)  # 128(up) + 256(skip)

        self.up4 = nn.ConvTranspose1d(128, 128, kernel_size=4, stride=2, padding=1)
        self.dec4 = DoubleConv(256, 128, dropout=True)  # 128(up) + 128(skip)
    ...
    def forward(self, x):
        ...
        # --- ボトルネック ---
        # BiLSTMに入力するために次元を並び替える (N, C, L) -> (N, L, C)
        b = p4.permute(0, 2, 1)
        b, _ = self.bilstm(b)
        b = self.relu(b)
        # 元の次元順に戻す (N, L, C) -> (N, C, L)
        b = b.permute(0, 2, 1)

        # --- デコーダ ---
        d1 = self.up1(b)
        d1 = self.crop_and_concat(s4, d1)
        d1 = self.dec1(d1)

        d2 = self.up2(d1)
        d2 = self.crop_and_concat(s3, d2)
        d2 = self.dec2(d2)

        d3 = self.up3(d2)
        d3 = self.crop_and_concat(s2, d3)
        d3 = self.dec3(d3)

        d4 = self.up4(d3)
        d4 = self.crop_and_concat(s1, d4)
        d4 = self.dec4(d4)
    ...
    def crop_and_concat(self, skip_tensor, up_tensor):
        """
        畳み込み演算によるわずかなサイズの違いを吸収するため、
        スキップ接続のテンソルをクロップ（中心を切り出し）して結合する。
        """
