...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        # x1がデコーダーからのテンソル、x2がエンコーダーからのスキップ接続
        x1 = self.upsample(x1)

        # サイズの差分を計算
        diff = x2.size(2) - x1.size(2)

        # x1（小さい方）の後ろに差分だけパディングを追加する
        # パディングは (左側のパディング数, 右側のパディング数) で指定
...
class Unet(nn.Module):
    def __init__(self, in_channels=16, out_channels=8, droprate=0.05):
        super().__init__()
        self.inc = DoubleConv(in_channels, 32, droprate)
        self.d1 = Down(32, 64, droprate)
        self.d2 = Down(64, 128, droprate)
        self.d3 = Down(128, 256, droprate)
        self.d4 = Down(256, 512, droprate)
        self.d5 = Down(512, 1024, droprate)  # ボトルネック層を追加

        # アップサンプリング層
        self.u1 = Up(1024, 512, droprate)
        self.u2 = Up(512, 256, droprate)
        self.u3 = Up(256, 128, droprate)
        self.u4 = Up(128, 64, droprate)
        self.u5 = Up(64, 32, droprate)  # 最後のアップサンプリング層を追加
    ...
    def forward(self, x):
        ...
        x4 = self.d3(x3)
        x5 = self.d4(x4)
        x6 = self.d5(x5)  # ボトルネック層を通過
        x = self.u1(x6, x5)
        x = self.u2(x, x4)
        x = self.u3(x, x3)
        x = self.u4(x, x2)
        x = self.u5(x, x1)  # 最後のアップサンプリング層を通過
        x = self.outc(x)
        return x
...
#         self.pool5 = nn.MaxPool1d(kernel_size=2)  # 追加
