...
def make_p_onset_extension_datas(
    ...
    r_onset = pt_array[5] + 5  # q_peakから5足した位置をR_onsetとする
    # print(ECG_datas)
...
def make_t_onset_extension_datas(
    ...
    r_offset = pt_array[7] - 5  # s_peakから5ms引いた位置をR_offsetとする
...
# def make_t_height_extension(PGV_datas,ECG_datas,pt_array,label_name,extation_rate):
#     t_onset_org=pt_array[3]
...
def make_p_height_extension(PGV_datas, ECG_datas, pt_array, label_name, extation_rate):
        ...
        for i in range(dataset_num):
            ...
            if not os.path.isfile(path) or not os.path.isfile(pt_path):
                ...
                if (
                    ...
                    t_onset = pt_array[3]
                    # 1. パラメータ設定
                    num_augmentations = 10  # 生成する拡張データの数
                    min_rate = 0.5  # 高さ倍率の最小値
                    max_rate = 2.0  # 高さ倍率の最大値

                    # 2. ランダムな高さ倍率を生成
                    # min_rate から max_rate までの範囲で、num_augmentations個の一様乱数を生成
                    extend_height_rates = (
                        torch.rand(num_augmentations) * (max_rate - min_rate) + min_rate
                    )

                    for rate in extend_height_rates:
                        # T波の高さ変更関数を呼び出し
                        (
                            ...
                            label_name_augment,
                            pt_array_augument,
                        ) = make_t_height_extension(
                            PGV_datas=PGV_train[0],
                            ECG_datas=ECG_train[0],
                            pt_array=pt_array,
                        ...
                        PGV_train_set.append(
                            PGV_train_augment_data.view(1, 16, datalength)
                        )
                        ECG_train_set.append(
                            ECG_train_augment_data.view(1, 8, datalength)
                        )
