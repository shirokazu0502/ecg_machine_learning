...
def plot_scatter_2d_pos(
    ...
    # fig.tight_layout()
    # fig.savefig(DIR_OUT + "lstmclustering_2d_test_sma" + str(SMA_NUM) + ".png")
    # plt.show()
...
def create_lstm_dataset(data, look_back=10):
    ...
    # look_backの範囲を使って入力データと出力データを作成
...
def main(args):
    ...
    train_dataset, test_dataset = Dataset_LSTM.Dataset_setup_8ch_pt_augmentation(
    ...
    # create_lstm_dataset(train_dataset[0][0], look_back=20)
    ...
    # input()
    # summary(lstm, input_size=(1,15 , 384))
    ...
    if args.mode == "train":
        ...
        for epoch in range(args.epochs):
            ...
            for iteration, (x, xo, label_name, pt_index) in enumerate(
                ...
                if args.loss_fn_type == "mse":
                    if args.loss_pt_on_off == "off":
                        ...
                        if (
                            ...
                            loss, mse, kdl = loss_fn_mse_PRT_QS(
                                recon_x, xo, mean, log_var, datalength, args, pt_index
                            )  # 0.8秒間ｓのデータ全体で損失計算,ただしQRSに重みをつける
                ...
                if iteration % args.print_every == 0:
                    ...
                    if args.conditional:
                        ...
                        ifx = lstm.inference(n=c.size(0), c=c)
                    else:
                        ifx = lstm.inference(n=16)
            ...
            test_val = []
            with torch.no_grad():
                for j, (x, xo, label_name, pt_index) in enumerate(train_data_loader):
                    x, xo = x.to(device), xo.to(device)
                    recon_x, mean, log_var, z = lstm(x)
                    for i in range(len(label_name)):
                        label_temps.append(label_name[i])
        ...
        if args.loss_fn_type == "bce":
            ...
            # torch.save(lstm.state_dict(), args.pth)
            plt.close()
        ...
        torch.save(lstm.state_dict(), args.pth)
        torch.save(lstm.state_dict(), os.path.join(args.fig_root, str(ts), args.pth))
        ...
        pth = args.pth
        lstm.load_state_dict(torch.load(pth, map_location=lambda storage, loc: storage))
        lstm.eval()
        z_temps = np.empty((0, latent_size))
        label_temps = []
        with torch.no_grad():
        ...
        lstm.eval()
        print(lstm)
        # names = torchvision.models.feature_extraction.get_graph_node_names(lstm)
        # print(names)
        # extractor = torchvision.models.feature_extraction.create_feature_extractor(lstm, ['encoder.MLP.AC0.weight', 'encoder.MLP.AC0.bias'])
        print(lstm.state_dict().keys())
        print("===========================")
        print("encorder_Linear1")
        print(lstm.encoder.linear_means.weight)
        print(lstm.encoder.linear_means.bias)
        print("===========================")
        record_loss_eval = []
        min_test_loss = 2000
        z_temps = np.empty((0, latent_size))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hidden1_size", type=int, default=400)
    parser.add_argument("--hidden2_size", type=int, default=400)
    ...
    parser.add_argument(
        "--fig_root", type=str, default=OUTPUT_DIR + "/" + "figs_newref"
    )  # 学習過程及びテスト結果を出力するフォルダ
    parser.add_argument("--conditional", action="store_true")
    parser.add_argument("--train_off", action="store_false")

    parser.add_argument(
        "--pth", type=str, default=r"lstm_cross_v2_5.pth"
    )  # 学習モデルのファイル名指定
