...
def analyze_waveform_properties():
    ...
    for subject_path in SUBJECT_DIRS:
        ...
        for i, dataset_file in enumerate(dataset_files):
            ...
            try:
                ...
                features["pr_interval_ms"] = (pt_array["q_peak"] - pt_array["p_onset"]) * ms_per_index
                features["qrs_duration_ms"] = (pt_array["s_peak"] - pt_array["q_peak"]) * ms_per_index
                features["qt_interval_ms"] = (pt_array["t_offset"] - pt_array["q_peak"]) * ms_per_index
                ...
                for peak_key in peak_keys:
                    ...
                    if 0 <= peak_idx < len(df_data):
                        ...
                        if ch_to_use:
                            features[f"amp_ch1_{peak_key}"] = df_data.loc[peak_idx, ch_to_use]
                        else:
                            features[f"amp_ch1_{peak_key}"] = np.nan
                            print(f"      WARNING: Neither 'ch_1' nor 'ch_16' found for {subject_name} heartbeat {i}. Amplitude for 15ch data set to NaN.")
                    else:
                ...
                print(f"    - Could not process file {os.path.basename(dataset_file)}. Error: {e}")
    ...
    subject_summary.columns = ['_'.join(col).strip() for col in subject_summary.columns.values]
