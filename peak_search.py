...
# 1. CSVデータを読み込む
df = pd.read_csv('../data/raw/extract_patient_data/patient2_12ch_20000.csv')  # CSVファイルのパスを指定してください
...
def main(args):
    ...
    if(handler.check_file()==False):#同期するためのファイルが存在していないとき。
        ...
        df_resample_12ch=linear_interpolation_resample_All(df=df_12ch_pf.copy(),sampling_rate=125.56,new_sampling_rate=RATE)
