...
# common_columns=[]
def update_normlized_data(frame):
    ax.clear()  # 軸をクリア
    sns_heatmap = sns.heatmap(np_reshape[:, :, frame], ax=ax, cmap='coolwarm', annot=False, vmax=1, vmin=0, cbar=False)

    # カラーバーを最初のフレームでのみ作成
    if cbar is None:
        cbar = fig.colorbar(sns_heatmap.collections[0], ax=ax, orientation="vertical")
    else:
        cbar.update_bruteforce(sns_heatmap.collections[0])  # 既存のカラーバーを更新

    plt.title(f'time(second): {frame}')  # RATE_15ch の値を frame に依存させています
    plt.xlabel('Column')
    plt.ylabel('Row')

def update_org(frame):
    ax.clear()
    sns_heatmap = sns.heatmap(np_org[:, :, frame], ax=ax, cmap='coolwarm', annot=True,cbar=False)
    print(frame)
...
def update_log(frame):
    ax.clear()
    sns_heatmap = sns.heatmap(np_reshape[:, :, frame], ax=ax, cmap='coolwarm', annot=False,vmax=1,vmin=0,cbar=True)
    print(frame)
...
if __name__ == "__main__":
    ...
    for j in range(12):
        ...
        df_keep["ch_16"]=0.0
        # print(df_keep)
        df_keep = df_keep.sub(df_keep["ch_1"], axis=0)
        # print(df_keep)
        # input("")
        list_dfs.append(df_keep)
    print(len(list_dfs))
    list_df_yoko=[]
    #１２個のシートが３×４（横×縦）で並んでいて一部の電極が重なっている。ここでは横方向に取っている。
    for i in range(4):
            df_3=list_dfs[i*3+2]
            df_2=list_dfs[i*3+1]
            df_1=list_dfs[i*3]
            ...
            # print(df_2)
            # print(df_3["ch_13"])
            df_2=df_2.add(df_3["ch_13"], axis='index')
            df_1=df_1.add(df_2["ch_13"], axis='index')
            # print(df_2)
            # input("")
    ...
    df_keep_7=df_keep_7.add(df_keep_12["ch_4"], axis='index')
    df_keep_8=df_keep_8.add(df_keep_12["ch_4"], axis='index')
    df_keep_9=df_keep_9.add(df_keep_12["ch_4"], axis='index')

    df_keep_4=df_keep_4.add(df_keep_9["ch_4"], axis='index')
    df_keep_5=df_keep_5.add(df_keep_9["ch_4"], axis='index')
    df_keep_6=df_keep_6.add(df_keep_9["ch_4"], axis='index')

    df_keep_1=df_keep_1.add(df_keep_6["ch_4"], axis='index')
    df_keep_2=df_keep_2.add(df_keep_6["ch_4"], axis='index')
    df_keep_3=df_keep_3.add(df_keep_6["ch_4"], axis='index')
    new_columns_list=[]
    for j in range(12):
        # new_columns_list.append({f'ch_{i+1}': "pos_{}_".format(j+1)+str(i//4+1)+'_'+str(i%4+1) for i in range(15)})
        new_columns_list.append({f'ch_{16-i}': str(i//4+1+(j%3)*3)+'_'+str(i%4+1+(j//3)*3) for i in range(16)})
    ...
    # sns.heatmap(np_reshape[:, :, 76],  cmap='coolwarm', annot=False, cbar=True  )
    # # # sns.heatmap(np_reshape[:, :, 48],  cmap='coolwarm', annot=False, cbar=True,vmax=1,vmin=0)
    # plt.show()
    # plt.savefig("T-wave")
    # plt.figure(figsize=(10, 13))  # ヒートマップのサイズを設定
    # sns.heatmap(np_reshape[:, :, 48],  cmap='coolwarm', annot=False, cbar=True  )
    # # # sns.heatmap(np_reshape[:, :, 48],  cmap='coolwarm', annot=False, cbar=True,vmax=1,vmin=0)
    # plt.show()
    # plt.savefig("R-wave")
    # print(np_reshape)
    ...
    # print()
    # print(np_reshape[0,0,:])
    # plt.plot(np_reshape[0,0,:])
    # plt.show()
    # input()

    # fig, ax = plt.subplots(figsize=(10, 13))
    # # アニメーションの作成
    # ani = animation.FuncAnimation(fig, update_org, frames=range(np_org.shape[2]), interval=100)
    # ani.save('heatmap_animation_org.mp4', writer='ffmpeg', fps=10)
    # fig, ax = plt.subplots(figsize=(10, 13))
    # cbar= None
    # # アニメーションの作成
    # ani = animation.FuncAnimation(fig, update_normlized_data, frames=range(np_reshape.shape[2]), interval=100)
    # ani.save('heatmap_animation.mp4', writer='ffmpeg', fps=10)

    print(np_reshape)
    fig, ax = plt.subplots(figsize=(10, 13))
    def init():
    #    sns_heatmap = sns.heatmap(np_reshape[:, :, 0], ax=ax, cmap='coolwarm', annot=False, vmax=max_value, vmin=min_value, cbar=True, cbar_ax=cbar_ax)
       sns_heatmap = sns.heatmap(np_reshape[:, :, 0], ax=ax, cmap='coolwarm', annot=False, vmax=10, vmin=-40, cbar=True, cbar_ax=cbar_ax)
       return [sns_heatmap]

    def update_norm(frame):
       ax.clear()  # 軸をクリア
       print(frame)
       sns_heatmap = sns.heatmap(np_reshape[:, :, frame], ax=ax, cmap='coolwarm', annot=False, vmax=10, vmin=-40, cbar=False)
    #    plt.suptitle(f'time(second): {frame/RATE_15CH}')  # RATE_15ch の値を frame に依存させています
       ax.set_title(f'time(second): {frame/RATE_15CH}', y=1.05)
    #    ax.set_title(f'time(second): {frame}', loc='center', pad=20)
       plt.xlabel('Column')
       plt.ylabel('Row')
       return [sns_heatmap]

    cbar_ax = fig.add_axes([0.85, 0.15, 0.05, 0.7])
    ani = animation.FuncAnimation(fig, update_norm, init_func=init, frames=range(1, np_reshape.shape[2]), interval=100, blit=False)
    plt.subplots_adjust(top=0.85,right=0.8)  # カラーバーのためにメインのプロットを右に移動
    # アニメーションの保存
    # ani.save('heatmap_animation.mp4', writer='ffmpeg', fps=10)
    ani.save('heatmap_animation_aaaa.mp4', writer='ffmpeg', fps=10)
